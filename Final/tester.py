...
if __name__ == '__main__':
    ...
    testrobot = Robot(testmaze.dim, mode)

    # Record robot performance over two runs.
    runtimes = []
    #TO-DO: excluir!!
    log = open('tester.txt','w')
    total_time = 0
    for run in range(2):
        ...
        while run_active:
            ...
            log.write(str(robot_pos['location']))
            log.write('.')
            # check for a reset
            ...
            if (rotation, movement) == ('Reset', 'Reset'):
                if run == 0 and hit_goal:
                    ...
                    print "Ending first run."                  
                    print "Taxa de exploracao: {:4.2f}%".format(testrobot.rateVisited())
    ...
    if len(runtimes) == 2:
        ...
        print "Numero de passos no treinamento: {0:d} ".format(runtimes[0])
        print "Numero de passos na corrida: {0:d}".format(runtimes[1])
        print "Taxa de exploracao: {:4.2f}%".format(testrobot.rateVisited())